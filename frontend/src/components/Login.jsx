...
export default function Login({ onLogin }) {
  ...
  // STEP 1: Submit Email for OTP Generation
  const handleCredentialsSubmit = async (e) => {
    e.preventDefault();
    const cleanEmail = email.trim();
    if (!cleanEmail) {
      setError(role === 'officer' ? 'Please enter your authorized officer email address.' : 'Please enter your registered email or Student ID.');
      return;
    }
    ...
    try {
      const res = await handleInitiateLogin(cleanEmail, 'email', role);
      if (res.status === 'otp_required') {
        setSessionId(res.session_id);
        setMaskedEmail(res.email_masked || cleanEmail);
        setCooldown(res.cooldown_seconds || 60);
        setStep('otp');
        setSuccessMsg(res.message || 'OTP verification code sent to your registered email.');
      } else if (res.token && onLogin) {
        onLogin(res.user);
      }
    } catch (err) {
      setError(err.message || 'Authentication failed. Please verify your email and role.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleOtpPaste = (e) => {
    ...
    handleOtpPasteString(pastedData);
  };

  const handleOtpPasteString = (pastedStr) => {
    const numericStr = pastedStr.replace(/\D/g, '').slice(0, 6);
    if (!numericStr) return;

    const newDigits = ['', '', '', '', '', ''];
    for (let i = 0; i < numericStr.length; i++) {
      newDigits[i] = numericStr[i];
    }
    setOtpDigits(newDigits);

    const nextFocusIdx = Math.min(numericStr.length, 5);
    otpInputRefs.current[nextFocusIdx]?.focus();

    if (numericStr.length === 6) {
      submitFullOtp(numericStr);
    }
  };

  const submitFullOtp = async (otpCode) => {
    setLoading(true);
    setError('');

    try {
      const res = await handleVerifyOtp(sessionId, otpCode);
      if (onLogin && res.user) {
        onLogin(res.user);
      }
    } catch (err) {
  ...
  const handleResendClick = async () => {
    ...
    try {
      ...
      setCooldown(res.cooldown_seconds || 60);
      setOtpDigits(['', '', '', '', '', '']);
      setSuccessMsg('A new 6-digit verification code has been sent to your registered email.');
      otpInputRefs.current[0]?.focus();
    } catch (err) {
                ...
                <form onSubmit={handleCredentialsSubmit} className="space-y-4">
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-[#CBD5E1] mb-1.5">
                      {role === 'officer' ? 'OFFICER EMAIL ADDRESS' : 'EMAIL ADDRESS OR STUDENT ID'}
                    </label>
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder={role === 'officer' ? 'Enter authorized officer email' : 'e.g. pavankumar.1280@gmail.com or STU1280'}
                        disabled={loading}
              ...
              <div className="space-y-5 animate-fadeIn">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] font-semibold uppercase tracking-wider text-[#A78BFA] bg-[#7C3AED]/20 border border-[#7C3AED]/40 px-2.5 py-1 rounded-full font-mono">
                    STEP 2 OF 2 &middot; OTP VERIFICATION
                  </span>
                </div>
