...
export function PlacementProvider({ children, initialUser = null }) {
  ...
  // Auth Operations
  const handleInitiateLogin = async (email, channel = 'email', role = 'student') => {
    setLoading(true);
    try {
      const res = await api.loginInit(email, channel, role);
      return res;
    } finally {
      setLoading(false);
    }
  ...
  const handleLogin = async (email, channel = 'email', role = 'student') => {
    return handleInitiateLogin(email, channel, role);
  };
  ...
  const sendCopilotMessage = async (text) => {
    ...
    try {
      const res = await api.chatWithAgent(text, {
        ...
        student_id: currentUser?.student_id || studentProfile?.id
      });
