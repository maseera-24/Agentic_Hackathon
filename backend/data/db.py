...
class Database:
    ...
    def add_students_batch(self, added_students: list, updated_students: list):
        """Batch upsert students from Excel upload with MongoDB sync and user account provisioning."""
        ...
        # 3. Synchronize login accounts in users collection
        for s in updated_students + added_students:
            sid = s.get("id")
            semail = (s.get("email") or "").strip().lower()
            sname = s.get("name")
            user_found = False
            for u in self.state.get("users", []):
                if u.get("student_id") == sid or (u.get("email", "").strip().lower() == semail and semail):
                    u["name"] = sname
                    u["email"] = semail
                    u["student_id"] = sid
                    user_found = True
                    break
            if not user_found and (sid or semail):
                self.state.setdefault("users", []).append({
                    "id": f"USR_{sid or uuid.uuid4().hex[:8].upper()}",
                    "email": semail,
                    "student_id": sid,
                    "name": sname,
                    "role": "student",
