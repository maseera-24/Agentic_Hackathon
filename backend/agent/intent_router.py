...
class IntentRouter:
    ...
    def classify_intent(self, message: str, role: str = "placement_officer", context: dict = None) -> Dict[str, Any]:
        ...
        if role == "student" or context.get("user_role") == "student":
            if any(w in m_lower for w in ["what drive", "can i apply", "eligible drive", "available drive", "open drive", "which drive"]):
                return {"intent": "STUDENT_ELIGIBLE_DRIVES", "params": params, "confidence": 0.95}
            if any(w in m_lower for w in ["my application", "applied drive", "drives applied", "applied to", "submitted application"]):
                return {"intent": "STUDENT_APPLIED_DRIVES", "params": params, "confidence": 0.95}
            if any(w in m_lower for w in ["my interview", "interview schedule", "interview slot", "upcoming interview", "when is my interview"]):
                return {"intent": "STUDENT_INTERVIEW_SCHEDULE", "params": params, "confidence": 0.95}
            if any(w in m_lower for w in ["my result", "placement result", "am i selected", "offer status", "selection result"]):
