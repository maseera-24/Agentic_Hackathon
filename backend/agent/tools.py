...
>>>>>>> 7ea430ac41087f03137a7143ffe3d545e060af90
    ...
    def student_get_eligible_drives(self, student_id: str):
        student = db.get_student(student_id) or db.get_students()[0]
        drives = db.get_drives()
        eligible_drives = []
        ineligible_drives = []
        for d in drives:
    ...
    def student_get_interview_schedule(self, student_id: str):
        apps = db.get_applications(student_id=student_id)
        interviews = [a for a in apps if a.get("interview_details", {}).get("date")]
        return {
            "student_id": student_id,
    ...
    def student_get_results(self, student_id: str):
        apps = db.get_applications(student_id=student_id)
        results = [a for a in apps if a.get("status") in ["SELECTED", "NOT_SELECTED"]]
        return {
            "student_id": student_id,
            "results": results
        }
